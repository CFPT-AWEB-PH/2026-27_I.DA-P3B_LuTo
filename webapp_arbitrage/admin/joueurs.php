...
<?php if (in_array($action, ['new', 'edit'])): ?>
    <div class="card" style="max-width:560px;">
        <h3><?= $action === 'new' ? 'Nouveau joueur' : 'Modifier le joueur' ?></h3>
        <form method="post" action="joueurs.php" class="wide">
            <input type="hidden" name="csrf" value="<?= e(csrfToken()) ?>">
            <input type="hidden" name="form_action" value="<?= $action === 'new' ? 'create' : 'update' ?>">
            <?php if ($action === 'edit' && !empty($editJoueur['id'])): ?><input type="hidden" name="id" value="<?= $editJoueur['id'] ?>"><?php endif; ?>

            <div class="field">
                <label for="pseudo">Pseudo (nom de combat) *</label>
                <input type="text" id="pseudo" name="pseudo" required autofocus value="<?= e($editJoueur['pseudo'] ?? '') ?>">
            </div>

            <div class="grid grid-2">
                <div class="field">
                    <label for="prenom">Prénom</label>
                    <input type="text" id="prenom" name="prenom" autocomplete="given-name" value="<?= e($editJoueur['prenom'] ?? '') ?>">
                </div>
                <div class="field">
                    <label for="nom">Nom</label>
                    <input type="text" id="nom" name="nom" autocomplete="family-name" value="<?= e($editJoueur['nom'] ?? '') ?>">
                </div>
            </div>
            ...
            <div class="grid grid-2">
                ...
                <div class="field">
                    ...
                    <small class="hint"><?= $age !== null ? 'Âge calculé automatiquement : ' . $age . ' ans' : "L'âge est calculé automatiquement, aucune saisie manuelle." ?></small>
                </div>
            </div>
            ...
            <div class="grid grid-2">
                ...
                <div class="field">
                    ...
                    <small class="hint">La liste des grades se configure dans Admin → Paramètres.</small>
                </div>
            </div>
...
<?php else: ?>
    <p class="btn-row">
        ...
        <a href="joueurs.php?action=export&csrf=<?= e(csrfToken()) ?>" class="btn btn-secondary"><?= icon('download') ?> Exporter en CSV (Excel)</a>
        <a href="joueurs.php?action=import" class="btn btn-secondary"><?= icon('upload') ?> Importer un CSV (Excel)</a>
    </p>
<?php endif; ?>

<?php if ($action === 'import'): ?>
    <div class="card" style="max-width:640px;">
        <h3><?= icon('upload') ?> Importer des joueurs depuis un CSV</h3>
        <p>
            Le fichier doit contenir les colonnes suivantes (dans n'importe quel ordre) :
            <code>pseudo</code>, <code>nom</code>, <code>prenom</code>, <code>sexe</code> (M/F/Autre),
            <code>date_naissance</code> (AAAA-MM-JJ), <code>poids</code>, <code>grade</code> (nom exact d'un grade existant).
        </p>
        <p>
            <strong>Le pseudo sert de clé</strong> : si un joueur avec ce pseudo existe déjà, sa fiche est mise à jour ;
            sinon, un nouveau joueur est créé automatiquement.
        </p>
        <p><a href="joueurs.php?action=export&csrf=<?= e(csrfToken()) ?>">Exporter d'abord la liste actuelle</a> donne un modèle de fichier déjà au bon format, à réimporter après modification.</p>
        <form method="post" action="joueurs.php" enctype="multipart/form-data" class="wide">
            <input type="hidden" name="csrf" value="<?= e(csrfToken()) ?>">
...
<div class="table-wrap">
    <table>
        <caption class="visually-hidden">Liste de tous les joueurs enregistrés, avec leur profil complet</caption>
        <thead>
            <tr>
        ...
        <?php foreach ($joueurs as $j): $age = calculerAge($j['date_naissance']); ?>
            <tr>
                ...
                <td class="actions-inline">
                    <a href="joueurs.php?action=edit&id=<?= $j['id'] ?>" class="btn btn-secondary">Éditer<span class="visually-hidden"> le joueur <?= e($j['pseudo']) ?></span></a>
                    <a href="joueurs.php?action=delete&id=<?= $j['id'] ?>&csrf=<?= e(csrfToken()) ?>"
                       class="btn btn-danger"
                       onclick="return confirm('Supprimer le joueur <?= e(addslashes($j['pseudo'])) ?> ?');">Supprimer<span class="visually-hidden"> le joueur <?= e($j['pseudo']) ?></span></a>
                </td>
            </tr>
        <?php endforeach; ?>
        <?php if (!$joueurs): ?>
            <tr><td colspan="11" class="empty-state">Aucun joueur enregistré.</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
</div>
